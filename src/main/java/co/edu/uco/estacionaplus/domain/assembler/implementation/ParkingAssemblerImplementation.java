...
public class ParkingAssemblerImplementation implements ParkingAssembler
{
    ...
    {
        return Parking.create(entity.getCode(), entity.getNit(), entity.getName(), entity.getAddress(), getCityAssembler().assembleDomainFromEntity(entity.getCity()), getParkingPlaceAssembler().assembleDomainsFromEntities(entity.getPlaces()));
    }

    @Override
    public ParkingEntity assembleEntityFromDomain(Parking domain)
    {
        return new ParkingEntity(domain.getCode(), domain.getNit(), domain.getName(), domain.getAddress(), getCityAssembler().assembleEntityFromDomain(domain.getCity()), getParkingPlaceAssembler().assembleEntitiesFromDomains(domain.getPlaces()));
    }

    @Override
    public Parking assembleDomainFromDTO(ParkingDTO dto)
    {
        return Parking.create(dto.getCode(), dto.getNit(), dto.getName(), dto.getAddress(), getCityAssembler().assembleDomainFromDTO(dto.getCity()), getParkingPlaceAssembler().assembleDomainsFromDTOs(dto.getPlaces()));
    }

    @Override
    public ParkingDTO assembleDTOFromDomain(Parking domain)
    {
        return new ParkingDTO(domain.getCode(), domain.getNit(), domain.getName(), domain.getAddress(), getCityAssembler().assembleDTOFromDomain(domain.getCity()), getParkingPlaceAssembler().assembleDTOsFromDomains(domain.getPlaces()));
    }

    @Override
    public ParkingEntity assembleEntityFromDomainToSave(Parking domain, City city) {
        return new ParkingEntity(domain.getCode(), domain.getNit(), domain.getName(), domain.getAddress(), getCityAssembler().assembleEntityFromDomain(city), getParkingPlaceAssembler().assembleEntitiesFromDomains(domain.getPlaces()));
    }

    @Override
    public ParkingEntity assembleEntityFromDomainToModify(int code, Parking domain) {
        return new ParkingEntity(code, domain.getNit(), domain.getName(), domain.getAddress(), getCityAssembler().assembleEntityFromDomain(domain.getCity()), getParkingPlaceAssembler().assembleEntitiesFromDomains(domain.getPlaces()));
    }
